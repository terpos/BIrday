...
void Fisher::react(Image &image, Sound sound, Player* & player, std::vector<E_Weapon*>& eweapon)
{
	if ((player->get_x() < get_x() + 240 && player->get_x() + 240 > get_x() && player->get_y() < get_y() + 240 && player->get_y() + 240 > get_y()) && get_vel() > 0)
	{
		al_set_sample_instance_position(sound.sound_effects(19), 0);
...
void Fisher::shoot(std::vector <E_Weapon*> &eweapon, Sound sound, Image spritesheet)
{
